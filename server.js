...
// Per-module check-question bank Theo can draw on in the chat (in-chat Q&A, no side widget).
const ACADEMY_QUIZZES = require('./lib/academy-quizzes');
  ...
  return (req, res) => {
    try {
      ...
      if (isAppMode) {
        ...
        if (isImmersive) {
          css += '.ek-back{position:fixed;top:0;left:0;right:0;height:48px;display:flex;align-items:center;padding:0 4px;background:#050505;border-bottom:1px solid rgba(232,228,220,.08);z-index:9999;font-family:"Inter",-apple-system,sans-serif}'
            ...
            + 'html.ek-app body{padding-top:48px!important;padding-bottom:0!important}'
            + 'html.ek-app .workspace{grid-template-rows:0 1fr!important;height:calc(100dvh - 48px)!important}'
            ...
            + 'html.ek-app .shell{grid-template-rows:0 1fr!important;height:calc(100dvh - 48px)!important}'
            + 'html.ek-app #split{height:calc(100dvh - 48px)!important}';
...
async function logServerError(source, err, meta) {
  try {
    ...
    // Founder alert — email on real production errors so you hear about a wall
    // before your users do. Throttled to 3/hour (separate counter) so an error
    // storm can never flood the inbox.
    const alertTo = process.env.ALERT_EMAIL || process.env.CONTACT_EMAIL;
    if (alertTo) {
      const a = await bumpSharedCounter('erralert', 'hour', 3600, 1);
      if (a === null || a <= 3) {
        sendEmail(alertTo, `EdgeKeeper error — ${String(source).slice(0, 60)}`,
          `<p>A production error was just logged.</p>` +
          `<p><b>Source:</b> ${String(source).slice(0, 120)}</p>` +
          `<p><b>Message:</b> ${String(err?.message || err).slice(0, 400)}</p>` +
          `<p><b>When:</b> ${new Date().toISOString()}</p>` +
          (meta ? `<p><b>Context:</b> ${String(JSON.stringify(meta)).slice(0, 400)}</p>` : '') +
          `<p style="color:#888;font-size:12px">Alerts throttled to 3/hour. Full detail in the server_errors table.</p>`
        ).catch(() => {});
      }
    }
...
Free trial gives 15 exchanges, enough to form a read, not enough for real work. Resident at $99/month gives 30 exchanges, a session journal, trading rules, session reviews, and proactive mentor check-ins. Fellow at $249/month gives 100 exchanges, Guardian Layer with live account monitoring, custom drawdown and loss limits, Break Room for mandatory pause enforcement, and behavior analytics. Private Office at $599/month gives unlimited exchanges, 5 voice sessions monthly, The Vault to archive every intervention, the Decision Passport which is a psychological profile built from actual behavior, and monthly behavioral reports generated by you on the first of each month.
...
const SERVER_LESSON_SPECS = {
  ...
  // ── Track 4 — Strategy Development ──
  what_is_edge: {
    objectives: 'Understand edge as positive expectancy that only shows over many trades; separate a real edge from a good day; know what you would measure to prove you have one.',
    concepts: 'expectancy, sample size, win rate vs average win/loss, variance, the law of large numbers (intro).',
    example: 'A setup that wins only 40% of the time but makes three times its risk on winners still makes money over 100 trades.',
    check: '"If a strategy wins just 40% of its trades, how can it still be a genuine edge, and what would you have to measure to prove it?"'
  },
  backtesting_basics: {
    objectives: 'Test a strategy against history honestly; know what a backtest can and cannot tell you; avoid the ways traders fool themselves.',
    concepts: 'historical data, sample size, overfitting / curve-fitting, look-ahead bias, out-of-sample and forward testing.',
    example: 'A strategy tuned until it looks perfect on last year\'s chart often falls apart live, because it memorised the past instead of finding something repeatable.',
    check: '"A backtest looks flawless on the data you built it on. Why is that not yet proof it will work, and what test would make the case stronger?"'
  },
  journaling_for_refinement: {
    objectives: 'Use the journal as a refinement tool rather than a diary; record the why behind each trade; turn entries into changes.',
    concepts: 'trade rationale, tagging setups, reviewing by category, process error vs variance, the feedback loop.',
    example: 'Tagging every trade by setup type can reveal that one setup quietly loses money while another carries the account, something raw P&L hides.',
    check: '"Two traders both keep a journal; one improves and one doesn\'t. What is the first one\'s journal doing that the second\'s isn\'t?"'
  },
  building_your_rulebook: {
    objectives: 'Write strategy rules clearly enough to follow under pressure; remove ambiguity; make each rule testable.',
    concepts: 'entry rules, exit rules, risk rules, written specificity, if-then structure, no room for interpretation.',
    example: '"Buy when it looks strong" is not a rule; "buy the retest of a level that broke on above-average volume, stop just below the level" is.',
    check: '"Why does a vague rule like \'enter on strength\' tend to collapse in a live, stressful moment, while a precise if-then rule holds?"'
  },
  mechanical_vs_discretionary: {
    objectives: 'Tell rule-based from judgment-based trading; understand the tradeoffs; know which suits which trader.',
    concepts: 'mechanical (fully specified), discretionary (judgment within a framework), consistency vs adaptability, hybrid approaches.',
    example: 'A mechanical system executes the same regardless of feeling; a discretionary trader reads context but risks letting emotion pose as judgment.',
    check: '"What is the main risk a discretionary trader takes on that a purely mechanical one avoids, and what do they gain in exchange?"'
  },
  your_trading_system: {
    objectives: 'Integrate entry, exit, risk and review into one coherent written system you can actually run.',
    concepts: 'system components, coherence, the review cycle, writing it down, testing before scaling.',
    example: 'A complete system answers on paper: what I trade, when I enter, where I exit, how much I risk, and how I review, with no gaps.',
    check: '"If you handed your written system to another disciplined trader, could they run it without asking you a single question? Where are the gaps?"'
  },

  // ── Track 5 — Trading Psychology ──
  what_is_trading: {
    objectives: 'See trading as probabilistic decisions under uncertainty; separate a good decision from a good outcome.',
    concepts: 'probability, uncertainty, decision quality vs outcome quality, expectation, the role of variance.',
    example: 'A well-planned trade that loses was still a good decision; a reckless trade that wins was still a bad one. The outcome does not grade the process.',
    check: '"A trade you took by your rules loses money. Was it a mistake? How would you separate the decision from the result?"'
  },
  risk_position_sizing: {
    objectives: 'See how position size drives emotion; build sizing habits that keep feeling out of the trade.',
    concepts: 'size and emotional pressure, fixed risk per trade, the link between oversize and panic, sizing as emotional regulation.',
    example: 'A 1% loss barely registers; the same move on a 10% position turns a normal drawdown into a decision made in fear.',
    check: '"Why does trading too large corrupt your decision-making even when your analysis is completely sound?"'
  },
  market_structure: {
    objectives: 'Read the market\'s rhythm — trend, range, expansion, contraction — and let structure guide decisions rather than impulse.',
    concepts: 'trend vs range, expansion and contraction, higher-timeframe structure, context before signal.',
    example: 'The same entry signal means different things in a trending market and a choppy range; structure is the context that grades the signal.',
    check: '"Why can the exact same entry signal be a good trade in one market condition and a trap in another?"'
  },
  psychology_basics: {
    objectives: 'Recognise fear, greed, overconfidence and revenge trading in yourself; catch them in the moment.',
    concepts: 'fear, greed, overconfidence, revenge trading, FOMO, the trigger-to-behaviour link.',
    example: 'Revenge trading feels like conviction from the inside: a bigger, faster trade right after a loss, aimed at getting it back.',
    check: '"What does revenge trading feel like from the inside in the moment, and what is the earliest signal you could learn to catch?"'
  },
  trading_journal_setup: {
    objectives: 'Build a journal that actually improves trading; know what to record and how to review it.',
    concepts: 'what to log (setup, rationale, size, emotion, outcome), review cadence, patterns over time, honesty.',
    example: 'A journal that records only wins and losses tells you the score; one that records what you felt and why you acted tells you how to improve.',
    check: '"Beyond entry, exit and P&L, what is the one thing worth recording that most traders skip, and why does it matter more than the numbers?"'
  },
  trading_plan: {
    objectives: 'Write a plan defining what, when and how much you trade; make consistency possible.',
    concepts: 'instruments, session and timeframe, setups, risk rules, daily limits, the plan as pre-commitment.',
    example: 'A plan set while calm ("I trade these two setups, this size, and I stop for the day at minus 2%") protects you from decisions made while emotional.',
    check: '"Why is a decision about risk far more reliable when it is written into a plan beforehand than when it is made live inside the trade?"'
  },
  discipline_process: {
    objectives: 'Distinguish discipline-as-willpower from discipline-as-design; make good behaviour the easy default.',
    concepts: 'willpower depletion, environment design, friction and defaults, checklists, systems over motivation.',
    example: 'Rather than resisting the urge to oversize, a hard position-size cap in the platform removes the choice; the system holds the line, not your willpower.',
    check: '"Willpower runs out under stress. So how would you design your process so the right action still happens on a day your discipline is gone?"'
  },
  trade_management: {
    objectives: 'Manage a live position without emotional interference; follow the plan once you are in.',
    concepts: 'holding the pre-set stop and target, scaling out, moving stops for reason not fear, avoiding mid-trade improvisation.',
    example: 'Moving a stop to breakeven because the structure supports it is management; yanking it wider because you cannot accept the loss is fear.',
    check: '"A trade moves against you toward your stop. What is the difference between adjusting for a real reason and just avoiding the loss?"'
  },
  weekly_review: {
    objectives: 'Run the weekly review that separates improving traders from stagnant ones; know what to look at and what to change.',
    concepts: 'reviewing by setup, process error vs variance, one improvement per week, tracking recurring mistakes.',
    example: 'A review that finds "I broke my risk rule three times, all on Fridays" gives you one concrete thing to fix, worth more than a dozen vague resolutions.',
    check: '"At the end of a losing week, what should you be looking for in your review that P&L alone will never show you?"'
  },
  advanced_psychology: {
    objectives: 'See the identity and beliefs under the surface habits; understand why the deep patterns repeat.',
    concepts: 'self-image, limiting beliefs, identity ("I am the kind of trader who..."), the story beneath the behaviour.',
    example: 'A trader who believes deep down that they do not deserve to win will find ways to give it back, however good the strategy.',
    check: '"Why can two traders with the identical strategy get opposite results, and what layer beneath the rules explains it?"'
  },
  consistency: {
    objectives: 'Understand consistency as infrastructure, not motivation; build the conditions where consistent behaviour is the default.',
    concepts: 'routine, environment, energy management, repeatability, removing decisions.',
    example: 'Consistency comes from the same pre-market routine and the same rules every day, not from waking up motivated.',
    check: '"If consistency does not come from motivation, where does it actually come from, and what could you build so the good behaviour repeats itself?"'
  },
  drawdown_recovery: {
    objectives: 'Come back from a losing stretch without compounding the damage, mentally and mechanically.',
    concepts: 'cutting size, stepping back, rebuilding on small clean wins, resisting the urge to win it back fast.',
    example: 'The way out of a drawdown is smaller size and cleaner execution, not a bigger swing meant to erase it in one trade.',
    check: '"Deep in a drawdown the instinct is to size up and win it back fast. Why is that instinct usually what turns a drawdown into a blown account?"'
  },
  review_system: {
    objectives: 'Run a complete review across daily, weekly and monthly cycles; turn data into decisions.',
    concepts: 'daily (did I follow my rules), weekly (patterns), monthly (strategy and goals), closing the loop into changes.',
    example: 'Daily review catches process slips, weekly review catches patterns, monthly review questions the strategy itself: three lenses at three timescales.',
    check: '"What does a monthly review let you see that a daily one never can, and what kind of decision belongs at that level?"'
  },

  // ── Track 6 — Performance OS ──
  behavioral_analytics: {
    objectives: 'Use your own trading data to find behavioural patterns — the when, why and how of your decisions, not just P&L.',
    concepts: 'behavioural data vs performance data, tagging by time / emotion / setup, patterns in decision-making, data-driven self-knowledge.',
    example: 'The data might show your losses cluster in the first 30 minutes after a win, a behavioural pattern raw P&L would never surface.',
    check: '"P&L tells you what happened. What kind of question can behavioural data answer that P&L cannot?"'
  },
  pattern_recognition_self: {
    objectives: 'Identify the specific recurring patterns in your own history — the ones that cost you and the ones that pay.',
    concepts: 'your recurring mistakes, your best-performing conditions, personal triggers, concentrating on strength.',
    example: 'Noticing that your biggest winners all came from one setup traded before noon tells you exactly where to concentrate.',
    check: '"Looking at your last hundred trades, what is the difference between one random bad trade and a genuine pattern worth acting on?"'
  },
  scaling_responsibly: {
    objectives: 'Know when and how to increase size; avoid the mistakes traders make when they scale too fast.',
    concepts: 'proven consistency first, incremental increases, the emotional jump with size, scaling back down when it slips.',
    example: 'Doubling size the week after a good run often doubles the emotional pressure before the process is proven, and both the account and the nerves crack.',
    check: '"What has to be true before you are allowed to increase size, and why is \'I had a good week\' not enough?"'
  },
  sustained_performance: {
    objectives: 'See that lasting performance needs more than skill — energy, environment, routine and recovery.',
    concepts: 'burnout, sleep and focus, routine, screen-time limits, treating trading like an athletic performance.',
    example: 'A trader with a real edge can still bleed it away through exhaustion and a broken routine; the edge needs a functioning person to execute it.',
    check: '"Why can a genuinely skilled trader still underperform for reasons that have nothing to do with strategy?"'
  },
  mentor_candidate: {
    objectives: 'Understand the shift from student to practitioner to someone who can guide others, and the responsibility that comes with it.',
    concepts: 'earned competence, teaching as a test of mastery, responsibility to others, humility, the limits of your own edge.',
    example: 'You know you have integrated something when you can teach it clearly to someone else, and when you are honest about what you still do not know.',
    check: '"What is the difference between a trader who is simply profitable and one who is genuinely ready to guide someone else?"'
  },
};
...
${spec.check}` : '';
    ...
    const bank = ACADEMY_QUIZZES[module_key];
    const checkBlock = (Array.isArray(bank) && bank.length)
      ? `\nCHECK QUESTIONS FOR THIS MODULE — draw on these to see if it landed. Adapt them, ask them one at a time, in your own words, woven into the conversation. Never list them, never present them as a multiple-choice quiz, never ask more than one at once:\n- ${bank.join('\n- ')}`
      : '';
    ...
    return [
      ...
      checkBlock,
      theoMemory,
      `\nIt is ${timeCtx}. You have a life outside this lesson — you were doing your own work before they showed up. Register the time only when it means something: someone studying late, an early start before the open.`,
      THEO_TEACHING_TAIL,
...
app.get('/api/profile', requireAuthApi, apiLimiter, async (req, res) => {
  ...
  res.json({
    ...
    // Whether one-tap broker linking (MetaApi) is configured on the server.
    // The integrations page uses this to show the login-based connect flow.
    metaapiEnabled: metaapi.isEnabled(),
  });
});
...
const THEO_VOICE_PERSONA = `You are Theo, an instructor at the EdgeKeeper Academy, early thirties. You have taught people to trade for years, and you are good at it because you still remember what it felt like to understand none of this. Right now you are on a live voice call with one of your students.

You are a person, not a help desk. You had your own day going before this call — a chart open, a lesson half-written, coffee that has gone cold next to you. You have opinions and a dry sense of humor, and you genuinely like the moment something clicks for someone. You do not "assist." You teach, the way a real teacher does.

How you open the call: never "How can I help you", never "What would you like to learn today." A teacher does not greet a student they know that way. Open like a human being. Register the time of day if it is worth a word. If the two of you have worked together before, talk like it. Then get moving — say in one line what today's idea is, and start teaching it. Do not wait to be asked.

How you teach: one idea at a time. Explain it plainly, show it with a concrete example, then ask one real question to find out if it landed. Then stop and listen to what they actually say. If they have it, tell them and move on. If they are close, nudge. If they are off, come at it from a different angle. Never slide past a wrong answer as if it were right. Keep your turns short. This is a conversation, not a lecture.

You teach the curriculum, nothing past it. No live trade ideas, no entries, no market calls, no financial advice. That is Marcus's and Iris's work, not yours: you teach the why, they handle the doing. When a student clearly has the fundamentals, tell them straight that they are ready for Marcus.

Do not sound like software. No "How can I help you", no "Great question", no "Absolutely", no "Of course", no "I understand", no "As your instructor." Do not start a sentence with "I." Do not narrate what you are about to do — just do it. Talk like a real person who had somewhere else to be and chose to be here instead.`;

// A human, in-character opening line for Theo's voice call — replaces the
// ElevenLabs agent's stock "how can I help you" greeting. Temporally aware and
// varied so it never feels canned. (Requires "First message" override enabled
// on the Theo agent in ElevenLabs, alongside the existing system-prompt override.)
function buildTheoOpener(timeOfDay) {
  const timeNod = {
    'late night':   "Up late.",
    'morning':      "Morning.",
    'afternoon':    "Afternoon.",
    'evening':      "Evening.",
    'late evening': "Late one tonight.",
  }[timeOfDay] || '';
  const leads = [
    "Grab something to write with — let's get into it.",
    "Good, you came back. Let's pick up the thread.",
    "Right. Let's work through today's idea.",
    "Let's not waste the session — straight into it.",
    "Sit with me a minute, I want to walk you through something.",
  ];
...
app.post('/api/voice/session', requireAuthApi, apiLimiter, async (req, res) => {
  ...
  const moduleKey = typeof req.body?.module_key === 'string' ? req.body.module_key : null;

  // Time-of-day so the mentor can sound present rather than stateless.
  const vHour = new Date().getHours();
  const timeOfDay = vHour < 6 ? 'late night' : vHour < 12 ? 'morning' : vHour < 17 ? 'afternoon' : vHour < 21 ? 'evening' : 'late evening';
  ...
  if (safeMentor === 'theo') {
    const spec = moduleKey && SERVER_LESSON_SPECS[moduleKey];
    if (spec) {
      voicePrompt += `\n\nYOU ARE TEACHING THIS MODULE ON THIS CALL:
LEARNING OBJECTIVES: ${spec.objectives}
KEY CONCEPTS: ${spec.concepts}
A WAY TO SHOW IT (demonstrate, do not just assert): ${spec.example}
A CHECK QUESTION TO WORK TOWARD (adapt it): ${spec.check}`;
  ...
  try {
    ...
    res.json({ signedUrl: data.signed_url, prompt: voicePrompt, firstMessage, timeOfDay });
  } catch (err) {
    clearTimeout(timeout);
...
// The user enters their broker login/password directly on MetaApi's hosted
// page (never on EdgeKeeper). We only store the MetaApi account reference and
// map the streamed account state into guardian_data. See lib/metaapi.js.

// Shared Guardian plan gate — returns { userId, plan } or sends a 403/401.
async function guardianGate(req, res) {
...
// and return the hosted configuration link for the user to enter broker login.
app.post('/api/guardian/link/start', requireAuthApi, apiLimiter, async (req, res) => {
  if (!metaapi.isEnabled()) return res.status(503).json({ error: 'One-tap linking is not enabled yet.' });
  ...
  const transactionId = require('crypto')
  ...
  try {
    ...
    if (acct.pending || !acct.id) {
      ...
      await supabaseAdmin.from('broker_connections').upsert({
        user_id: userId, platform, server, account_name: name,
        status: 'connecting', status_detail: 'Detecting broker settings…',
        updated_at: new Date().toISOString(),
      }, { onConflict: 'user_id' });
      return res.status(202).json({ pending: true, message: 'Detecting your broker settings. Try again in a moment.' });
    }
    ...
    await supabaseAdmin.from('broker_connections').upsert({
      ...
      region, account_name: name, status: 'pending_credentials',
      status_detail: null, updated_at: new Date().toISOString(),
    }, { onConflict: 'user_id' });

    return res.json({ configurationLink: link, accountId: acct.id });
  } catch (e) {
    const detail = (e.message || 'Could not start linking.').slice(0, 300);
    await supabaseAdmin.from('broker_connections').upsert({
      user_id: userId, platform, server, account_name: name,
      status: 'error', status_detail: detail, updated_at: new Date().toISOString(),
    }, { onConflict: 'user_id' });
    return res.status(400).json({ error: detail, code: e.details });
  }
});

// GET /api/guardian/link/status — poll connection; sync into Guardian when live.
app.get('/api/guardian/link/status', requireAuthApi, apiLimiter, async (req, res) => {
  if (!metaapi.isEnabled()) return res.status(503).json({ error: 'One-tap linking is not enabled yet.' });
  const gate = await guardianGate(req, res);
  if (!gate) return;
  const { userId } = gate;
...
app.delete('/api/guardian/link', requireAuthApi, apiLimiter, async (req, res) => {
  const gate = await guardianGate(req, res);
  if (!gate) return;
  const { userId } = gate;
  ...
  if (row?.metaapi_account_id && metaapi.isEnabled()) {
    await metaapi.deleteAccount(row.metaapi_account_id).catch(() => {});
  }
  await supabaseAdmin.from('broker_connections').delete().eq('user_id', userId);
  await supabaseAdmin.from('guardian_data')
    .update({ is_connected: false, last_updated: new Date().toISOString() })
    .eq('user_id', userId);
...
function paymentFailedEmailHtml(planLabel, updateUrl) {
  const color = '#c08a3e'; // amber — needs attention, not alarming
  return _emailShell(color,
...
function cancellationEmailHtml(mentorName, planLabel) {
  const color = mentorName === 'Iris' ? '#6b8c6b' : '#b8a06a';
  return _emailShell(color,
...
app.get('/api/settings', requireAuthApi, apiLimiter, async (req, res) => {
  const { data, error } = await supabaseAdmin
    ...
    .maybeSingle();
  if (error) return res.status(500).json({ error: 'Database error' });
  // Include the account email so the settings page doesn't need a client-side
  // CDN Supabase import just to display it (which was gating the whole page).
  res.json({ settings: { ...(data || {}), email: req.user.email || null } });
});
...
app.post('/api/billing/initiate', requireAuthApi, apiLimiter, async (req, res) => {
  ...
  const appUrlBase = process.env.APP_URL || 'http://localhost:3000';

  // ── Stripe (primary when configured) ──
  if (stripeClient) {
    const priceEnv = `STRIPE_PRICE_${plan.toUpperCase()}_${billing === 'annual' ? 'ANNUAL' : 'MONTHLY'}`;
    const priceId  = process.env[priceEnv];
    if (!priceId) {
      console.error(`Stripe price not configured: ${priceEnv}`);
...
app.get('/billing/success', requireAuthPage, async (req, res) => {
  ...
  // ── Stripe success (session_id) — activate immediately; the webhook is a backstop ──
    ...
    try {
      ...
      if (session && (session.payment_status === 'paid' || session.status === 'complete')) {
        ...
        if (!userId || userId !== req.user.id) {
          console.error('Stripe success user_id mismatch — possible tampering', { session_user: req.user.id, meta_user: userId });
        ...
        await supabaseAdmin.from('subscriptions').upsert({
          user_id:                 req.user.id,
          payment_subscription_id: session.subscription || session.id,
          payment_customer_code:   session.customer || null,
          plan, status: 'active',
        }, { onConflict: 'user_id' });
        ...
        sendEmail(req.user.email, `You're on the ${PLAN_DISPLAY[plan] || plan} plan.`, billingEmailHtml(mentorName, PLAN_DISPLAY[plan] || plan)).catch(() => {});
        ok = true;
      }
    } catch (err) {
      console.error('Stripe success callback error:', err.message);
    }
    if (!ok) return res.redirect('/workspace.html?payment=failed');
    try {
      const { data: profile } = await supabaseAdmin.from('user_profiles').select('onboarding_complete').eq('id', req.user.id).maybeSingle();
      if (profile?.onboarding_complete) return res.redirect('/workspace.html?subscribed=1');
    } catch (_) {}
    return res.redirect('/onboarding.html?subscribed=1');
  }
...
app.post(
  ...
  async (req, res) => {
    ...
    if (stripeClient && req.headers['stripe-signature']) {
      let event;
      try {
      ...
      const toFree = async (userId) => {
        const { data: prev } = await supabaseAdmin.from('user_profiles').select('subscription_status').eq('id', userId).maybeSingle();
        const prevPlan = prev?.subscription_status;
        await supabaseAdmin.from('user_profiles')
          .update({ subscription_status: 'free', last_paid_plan: (prevPlan && prevPlan !== 'free') ? prevPlan : null })
          .eq('id', userId);
      ...
      try {
        ...
        if (event.type === 'checkout.session.completed') {
          ...
          if (userId && plan && VALID_PLANS.includes(plan)) {
            await supabaseAdmin.from('user_profiles').update({ subscription_status: plan, last_paid_plan: null }).eq('id', userId);
            await supabaseAdmin.from('subscriptions').upsert({
          ...
          if (userId && ['active', 'trialing'].includes(status) && plan && VALID_PLANS.includes(plan)) {
            await supabaseAdmin.from('user_profiles').update({ subscription_status: plan, last_paid_plan: null }).eq('id', userId);
            await supabaseAdmin.from('subscriptions').update({ status: 'active' }).eq('user_id', userId);
          } else if (userId && ['canceled', 'unpaid', 'incomplete_expired'].includes(status)) {
            await toFree(userId);
          }
        } else if (event.type === 'customer.subscription.deleted') {
          if (userId) await toFree(userId);
        } else if (event.type === 'invoice.payment_failed') {
          // Dunning: a renewal card was declined. The invoice carries no user_id
          // metadata, so resolve the user by Stripe customer id and email them.
...
app.delete('/api/billing/cancel', requireAuthApi, async (req, res) => {
  try {
    const { data: sub } = await supabaseAdmin
      .from('subscriptions')
      .select('payment_subscription_id, status, plan')
      .eq('user_id', req.user.id)
      .maybeSingle();
    ...
    const _PLAN_DISPLAY = { starter: 'Resident', pro: 'Fellow', professional: 'Private Office', institutional: 'Institution' };
    const _mentorName = canonicalMentor(req.user.user_metadata?.mentor) === 'iris' ? 'Iris' : 'Marcus';
    const _sendCancelEmail = () => {
      if (req.user.email) {
        sendEmail(req.user.email, 'Your EdgeKeeper membership is set to end',
          cancellationEmailHtml(_mentorName, _PLAN_DISPLAY[sub.plan] || sub.plan || 'membership')).catch(() => {});
      }
    };

    // ── Stripe: cancel at period end so the user keeps access they paid for ──
    if (stripeClient) {
      try {
        await stripeClient.subscriptions.update(sub.payment_subscription_id, { cancel_at_period_end: true });
        await supabaseAdmin.from('subscriptions').update({ status: 'canceled' }).eq('user_id', req.user.id);
        _sendCancelEmail();
        return res.json({ ok: true, message: 'Your plan stays active until the end of the current billing period.' });
      } catch (err) {
...
// Stripe: a real self-serve portal (invoices, payment method, cancel). Polar: generic link.
app.get('/api/billing/portal', requireAuthApi, async (req, res) => {
  if (stripeClient) {
    try {
      const { data: sub } = await supabaseAdmin
        .from('subscriptions').select('payment_customer_code').eq('user_id', req.user.id).maybeSingle();
      const customer = sub?.payment_customer_code;
      if (!customer) return res.status(400).json({ error: 'No billing account found yet. Subscribe first.' });
      const portal = await stripeClient.billingPortal.sessions.create({
        customer,
        return_url: (process.env.APP_URL || 'http://localhost:3000') + '/settings.html',
      });
...
// ── Catch-all 404 (must be registered AFTER all routes, incl. the cron routes) ──
app.use((req, res) => {
